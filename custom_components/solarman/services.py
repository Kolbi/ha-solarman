...
SERVICE_READ_REGISTER_SCHEMA = vol.Schema(
    {
        ...
        vol.Required(SERVICES_PARAM_REGISTER): vol.All(
...
SERVICE_READ_MULTIPLE_REGISTERS_SCHEMA = vol.Schema(
    {
        ...
        vol.Required(SERVICES_PARAM_COUNT): vol.All(
            vol.Coerce(int), vol.Range(min=0, max=65535)
        ),
    }
)
...
SERVICE_WRITE_HOLDING_REGISTER_SCHEMA = vol.Schema(
    {
        ...
        vol.Required(SERVICES_PARAM_VALUE): vol.All(
            vol.Coerce(int), vol.Range(min=0, max=65535)
        ),
    }
)
...
SERVICE_WRITE_MULTIPLE_HOLDING_REGISTERS_SCHEMA = vol.Schema(
    {
        ...
        vol.Required(SERVICES_PARAM_VALUES): vol.All(
...
def register_services(hass: HomeAssistant) -> Inverter:
    _LOGGER.debug("register_services")
    ...
    def getInverter(device_id):
        ...
        for entity_reg in entries:
            ...
            if entity_comp is None:
                _LOGGER.info(
                    f"read_holding_register: Component for {entity_id} not loaded"
                )
                continue
            ...
            if (inverter := entity_obj.inverter) is None:
                _LOGGER.info(
                    f"read_holding_register: Entity {entity_id} has no inverter"
                )
                continue
    ...
    hass.services.async_register(
        ...
        schema=SERVICE_WRITE_HOLDING_REGISTER_SCHEMA,
    )
