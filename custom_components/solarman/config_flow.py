...
async def step_user_data_process(discovery):
    ...
    return {
        ...
        CONF_INVERTER_HOST: await discovery.get_ip(),
        CONF_INVERTER_SERIAL: await discovery.get_serial(),
        CONF_INVERTER_PORT: DEFAULT_PORT_INVERTER,
        CONF_INVERTER_MB_SLAVE_ID: DEFAULT_INVERTER_MB_SLAVE_ID,
        CONF_LOOKUP_FILE: DEFAULT_LOOKUP_FILE,
        CONF_BATTERY_NOMINAL_VOLTAGE: DEFAULT_BATTERY_NOMINAL_VOLTAGE,
        CONF_BATTERY_LIFE_CYCLE_RATING: DEFAULT_BATTERY_LIFE_CYCLE_RATING,
    }


async def step_user_data_schema(
    hass: HomeAssistant,
    data: dict[str, Any] = {
        CONF_NAME: DEFAULT_NAME,
        CONF_DISCOVERY: DEFAULT_DISCOVERY,
    ...
    lookup_files = sorted(
        [
            f
            for f in await async_execute(
                lambda: os.listdir(hass.config.path(LOOKUP_DIRECTORY_PATH))
            )
            if os.path.isfile(LOOKUP_DIRECTORY_PATH + f)
        ]
    )
    _LOGGER.debug(
        f"step_user_data_schema: data: {data}, {LOOKUP_DIRECTORY_PATH}: {lookup_files}"
    )
    STEP_USER_DATA_SCHEMA = (
        ...
        if wname
        else vol.Schema({}, extra=vol.PREVENT_EXTRA)
    )
    STEP_USER_DATA_SCHEMA = STEP_USER_DATA_SCHEMA.extend(
        {
            ...
            vol.Optional(
                CONF_INVERTER_MB_SLAVE_ID, default=data.get(CONF_INVERTER_MB_SLAVE_ID)
            ): int,
            vol.Optional(CONF_LOOKUP_FILE, default=data.get(CONF_LOOKUP_FILE)): vol.In(
                lookup_files
            ),
            vol.Optional(
                CONF_BATTERY_NOMINAL_VOLTAGE,
                default=data.get(CONF_BATTERY_NOMINAL_VOLTAGE),
            ): int,
            vol.Optional(
                CONF_BATTERY_LIFE_CYCLE_RATING,
                default=data.get(CONF_BATTERY_LIFE_CYCLE_RATING),
            ): int,
        }
    )
    _LOGGER.debug(
        f"step_user_data_schema: STEP_USER_DATA_SCHEMA: {STEP_USER_DATA_SCHEMA}"
    )
...
class ConfigFlowHandler(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(
        ...
        if user_input is None:
            discovery_options = await step_user_data_process(
                InverterDiscovery(self.hass)
            )
            return self.async_show_form(
                step_id="user",
                data_schema=await step_user_data_schema(self.hass, discovery_options),
            )
        ...
        try:
            ...
            _LOGGER.debug(
                f"ConfigFlowHandler.async_step_user: validation passed: {user_input}"
            )
        ...
        _LOGGER.debug(
            f"ConfigFlowHandler.async_step_user: validation failed: {user_input}"
        )

        return self.async_show_form(
            step_id="user",
            data_schema=await step_user_data_schema(self.hass, user_input),
            errors=errors,
        )
...
class OptionsFlowHandler(OptionsFlow):
    ...
    async def async_step_init(
        ...
        if user_input is None:
            return self.async_show_form(
                step_id="init",
                data_schema=await step_user_data_schema(
                    self.hass, self.entry.options, False
                ),
            )
        ...
        return self.async_show_form(
            step_id="init",
            data_schema=await step_user_data_schema(self.hass, user_input, False),
            errors=errors,
        )
