...
class SolarmanCoordinatorEntity(CoordinatorEntity[InverterCoordinator]):
    def __init__(self, coordinator: InverterCoordinator, name: str = None):
        ...
        self._attr_device_info = (
            {
                "connections": {
                    (CONNECTION_NETWORK_MAC, format_mac(self.coordinator.inverter.mac))
                }
            }
            if self.coordinator.inverter.mac
            else {}
            | {
...
class SolarmanEntity(SolarmanCoordinatorEntity):
    def __init__(self, coordinator, sensor):
        ...
        self.sensor_unique_id = (
            self.sensor_entity_id if self.sensor_entity_id else self.sensor_name
        )
        ...
        self._attr_name = "{} {}".format(
            self.coordinator.inverter.name, self.sensor_name
        )

        # Set the entity_id of the sensor.
        if self.sensor_entity_id:
            self.entity_id = "sensor.{}_{}".format(
                self.coordinator.inverter.name, self.sensor_entity_id
            )

        # Set a unique_id based on the serial number
        self._attr_unique_id = "{}_{}_{}".format(
            self.coordinator.inverter.name,
        ...
        if c > 1 or (c == 1 and self.sensor_name in self.coordinator.data):
            if self.sensor_name in self.coordinator.data:
                ...
                if "value" in self.coordinator.data[self.sensor_name]:
                    self._attr_extra_state_attributes["value"] = self.get_data_value(
                        self.sensor_name
                    )
                if self.attributes:
                    for attr in self.attributes:
                        if attr in self.coordinator.data:
                            ...
                            self._attr_extra_state_attributes[attr_name] = (
                                self.get_data_state(attr)
                            )
